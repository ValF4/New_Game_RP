...
local Remotes = ReplicatedStorage:WaitForChild("Remotes").RemoteFunctions.FireMoneyTranferer
...
Remotes.OnServerInvoke = function(Plr, PLAYER_CHAR, VALUE)
	if db[Plr.UserId] and tick() - db[Plr.UserId]  < .4 then return end
	db[Plr.UserId] = tick()

	local My_Data = ServerData.get(Plr)

	if My_Data.Money < VALUE then return CallNotificationServer:FireClient(Plr, "Transferencia não realizada:", "Você não possui o valor em mãos para transferencia.", "ERROR", 5) end
